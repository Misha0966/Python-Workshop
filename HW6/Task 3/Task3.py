# Планеты вращаются вокруг звезд по эллиптическим орбитам. Назовём самой далекой
...
# орбит планет найдет ту, по которой вращается самая далекая планета. Круговые орбиты
...
# При решении задач используйте списочные выражения.
...
def find_farthest_orbit(array):
    result = list(i for i in array if i[0] != i[1])
    area = list(i[0] * i[1] for i in result)
